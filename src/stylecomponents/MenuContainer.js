...
const MenuContainer = styled.aside`
  ...
  /* max-width: 540px; */
  height: 100%;
  /* ${(props) => (props.isShowingMore && props.isPortrait ? `position:absolute;` : ``)}; */
  overflow: auto;
  /* top:0px; */
  ${tablet} {
    top: 0px;
  }
  ${(props) => (props.isPortrait ? `padding: 0 ${navBarSize};` : `padding: 0 ${navBarSize} 0 0;`)};
  ...
  ${(props) =>
    props.navBarRight && props.isPortrait ? `padding-right:${navBarSize}; padding-left:0;` : `padding-right: 0;`}

  ${mainTransition}
	${(props) => (props.isPortrait ? `margin: 0 0 0;` : `margin: 0 ${navBarSize} 0 0 ;`)};
  ...
  /* ${(props) => (isIOS && props.isPortrait ? slightBoxShadowAbove : `box-shadow: none`)}; */
  .react-tabs {
    /* padding: 1rem 0; */
    ${paddingLRSm}
    &__tab-list {
  ...
  .subMenu {
    ...
    &__item {
      ...
      ${mobile} {
        ...
        ${(props) => (props.isShowingMore ? `margin: 0 0 .3rem` : `margin: .2rem 0 .3rem`)};
        margin-right: 1.5rem;
        a {
          /* ${tablet}{
  ...
  ${tablet} {
    ${(props) => (props.isPortrait ? `padding-left: ${navBarSize}; padding-right: ${navBarSize};` : ``)};
  }
  .alert-enter {
    opacity: 0;
    transform: scale(0.9);

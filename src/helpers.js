...
// 	.slice(0, 10)
// 	.replace(/-/g, ".");
export const appVersionNumber = `1.6`;
...
export function minSliderSize(width, height) {
  let minSize, maxSize;
  isPortrait(width, height) ? (minSize = height * sliderMinSizeMobile) : (minSize = 455);
  // isPortrait(width, height)
  // ? (minSize = height * sliderMinSizeMobile)
  // : (minSize = width * sliderMinSize);
  isPortrait(width, height) ? (maxSize = height - minSize) : (maxSize = width - minSize);
  return {
    minSize,
    maxSize,
  };
...
export function defaultPaneSize(ww, wh) {
  let defaultSize;
  isPortrait(ww, wh) ? (defaultSize = defaultContentHeight) : (defaultSize = defaultContentWidth);
  return defaultSize;
}
...
export function getCurrentTheme(theDefaultTheme, theTheme) {
  ...
  const merged = found ? Object.assign(theDefaultTheme, found.properties) : theDefaultTheme;
  ...
  const currentThemeObject = JSON.parse(localStorage.getItem("currentThemeObject"));
